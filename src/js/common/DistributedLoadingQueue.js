...
var logger = _req('src/js/logger').create('DistributedLoadingQueue', 'logs/loadingQueue.log');
...
function DistributedLoadingQueue() {
    this.clients = [];
    this.orphanTasks = [];
    this.resolvers = [];
...
DistributedLoadingQueue.prototype.manageOrphanTasks = function () {
    ...
    setInterval(function () {
        ...
        if (inst.orphanTasks.length && inst.clients.length && client) {
            logger.info('Managing orphan tasks.');
            while (client) {
...
DistributedLoadingQueue.prototype.resolveTask = function (data) {
    ...
    this.resolvers.every(function (resolver) {
        if (resolver.url == url) {
            ...
            logger.info('Task for url "%s" was resolved.', url);
            return false;
        }
        return true;
    });
...
module.exports = {
    ...
    instance: function (http) {
        if (!instance && !wsServer) {
            wsServer = _req('src/js/common/WSServer').instance(http.server);
