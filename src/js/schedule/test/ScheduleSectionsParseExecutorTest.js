...
var logger = _req('src/js/logger').create('ScheduleSectionsParseExecutorTest');
...
ScheduleSectionsParseExecutorTest.prototype.run = function(schedule, extend) {
    ...
    if (utils.hasContent(destIndex)) {
        logger.warn('Using default depth level "0" for testing purpose.');
        destIndex = 0;
    }
    if (isNaN(maxIteration)) {
        logger.warn('Using default max iteration "1" for testing purpose.');
        maxIteration = 1;
    }
