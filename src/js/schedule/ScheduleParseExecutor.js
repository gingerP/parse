...
var logger = _req('src/js/logger').create('ScheduleParseExecutor');
    ...
    return new Promise(function(resolve, reject) {
        if (!inst.schedule) {
            return inst.loadDependencies(inst._scheduleId).then(function() {
                ...
                inst.getExecutor()();
                logger.info('Scheduler "%s" started!', inst.getName());
                resolve(true);
            })
        } else {
            inst.getCronInstance().start();
...
ScheduleParseExecutor.prototype.updateStatus = function(status) {
    if (this.parseConfig) {
        ...
        logger.warn('UpdateStatus. There is now schedule config for schedule executor!');
    }
};
        ...
        utils.loadDom(inst.parseConfig.url, function(error, body) {
            ...
            try {
                ...
                parseDataDBManager.saveByCriteria(entity, {code: inst.parseConfig.code}).then(function (id) {
                    //TODO what if error?
                    logger.info('Scheduler "%s" was update data.', inst.getName());
