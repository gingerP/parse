...
var logger = _req('src/js/logger').create('DistributedParserClient', 'logs/loadingClientChild.log');
var utils = _req('src/js/utils');
var _ = require('lodash');
var REQUEST_COUNT_FOR_RESTART = 500;
var ERROR_COUNT_FOR_RESTART = 20;
logger.warn('---------------------------------------------------------------------------------------------------------');
logger.warn('New client (pid: %s)', process.pid);
...
DistributedParserClient.prototype.manageTaskResult = function (url, data) {
    logger.info('Task for url %s successfully finished!', url);
